...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    #Upscale from lay7 to pool layer 4
    deconv_1 = tf.layers.conv2d_transpose(conv_1X1, 512, 4, 2, padding='same',
                                kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    #print(deconv_1.shape())
    #Fuse with output from layer 4
    fuse_1 = tf.add(deconv_1, vgg_layer4_out)
    #Upscale from pool layer 4 to pool layer 3
    deconv_2 = tf.layers.conv2d_transpose(fuse_1, 256, 4, 2, padding='same',
                             kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    fuse_2 = tf.add(deconv_2, vgg_layer3_out)
    ##Upscale from pool layer 4 to orginal image size
    nn_last_layer = tf.layers.conv2d_transpose(fuse_2, num_classes, 16, 8, padding='same',
                                kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for epoch in range(epochs):
        ...
        for batch_images, batch_labels in get_batches_fn(batch_size):
            ...
            _, train_loss = sess.run([train_op,
                                   cross_entropy_loss],
...
def run():
    ...
    with tf.Session() as sess:
        ...
        #keep_prob = tf.float32(KEEP_PROB)
